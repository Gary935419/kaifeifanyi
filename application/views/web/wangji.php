	...
	<div class="mcenter">
		<div class="register_xiaopassword">
			<div class="register_section">
				...
				<span class="register_title"><?php echo $yuyan4['text15'] ?></span>
				<div action="" class="register_form layui-form">
					...
					<button class="register_btu" onclick="insertmenmber()"><?php echo $yuyan4['text16'] ?></button>
				</div>
			</div>
			<div class="loginnew">
				<?php if (empty($ltype)){ ?>
...
<script>
	...
	function insertmenmber() {
		$.ajax({
			cache: true,
			type: "POST",
			url: "<?= RUN . '/webviews/insertmemberwangji' ?>",
			data: {email:$('#youxiang').val(),yanzhengma:$('#yanzhengma').val(),mima:$('#mima').val(),querenmima:$('#querenmima').val(),leixing:$('#leixing').val()},
			async: false,
			error: function (request) {
				alert("error");
			},
