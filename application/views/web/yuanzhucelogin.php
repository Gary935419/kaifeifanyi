		...
		<div class="register_xiaopassword">
			<div class="register_section">
				...
				<div action="" class="register_form layui-form">
					...
					<a href="<?= RUN . '/webviews/wangji?leixing=2' ?>" class="register__login"><?php echo $yuyan4['text15'] ?></a>
				</div>
			</div>
			<div class="loginnew">
    		<?php if (empty($ltype)){ ?>
    			<a href="<?= RUN . '/webviews/yuanzhucelogin?ltype=2' ?>" class="search_login">
    				<span>English</span>
    			</a>
    		<?php }else{ ?>
    			<a href="<?= RUN . '/webviews/yuanzhucelogin?ltype=1' ?>" class="search_login">
    				<span>中文</span>
    			</a>
    		<?php } ?>
...
<script>
	...
	function insertmenmber() {
		$.ajax({
			cache: true,
			type: "POST",
			url: "<?= RUN . '/webviews/insertmemberlogin1' ?>",
			...
			success: function (data) {
				...
				if (data.result){
					...
					if (data.status == 1){
						...
						layer.msg(data.msg);
						setTimeout(function(){
							window.location.href='/webviews/membernongchang';
						},1000);
					}
				}else {
					layer.msg(data.msg);
