				...
				<div class="layui-card-body ">
					<form class="layui-form layui-col-space5" method="get" action="<?= RUN, '/goods/goods_list1' ?>">
						<div class="layui-inline layui-show-xs-block">
							<input type="text" name="mingcheng" id="mingcheng" value="<?php echo $mingcheng ?>"
								   placeholder="咖啡店名称或电话或邮箱" autocomplete="off" class="layui-input">
						</div>
						<div class="layui-inline layui-show-xs-block">
								...
								<tr id="p<?= $once['id'] ?>" sid="<?= $once['id'] ?>">
									...
									<td class="td-manage">
										<button class="layui-btn layui-btn-normal"
												onclick="xadmin.open('编辑','<?= RUN . '/goods/goods_edit1?id=' ?>'+'<?= $once['id'] ?>',1000,600)">
											<i class="layui-icon">&#xe642;</i>编辑
										</button>
										<button class="layui-btn layui-btn-normal"
												onclick="xadmin.open('翻译','<?= RUN . '/goods/goods_edit1?id=' ?>'+'<?= $once['idf'] ?>',1000,600)">
		...
		layer.confirm('您是否确认更改显示状态？', {
				...
				function (index) {
					$.ajax({
						...
						dataType: "json",
						url: "<?= RUN . '/goods/goods_delete_state1' ?>",
						success: function (data) {
							if (data.success) {
								layer.alert(data.msg, {
											title: '温馨提示',
